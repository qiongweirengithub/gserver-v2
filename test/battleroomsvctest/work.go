...
func NewWork(manager *Manager) *Work {
	...
	opts := this.GetDefaultOptions("ws://47.112.159.211:3653")
...
func (this *Work) RunWorker(t task.Task) {
	...
	msg, err := this.Request("battleroomsvc@12345/HD_create_table", []byte(`{"table_id":"testtable01"}`))

	msg, err = this.Request("battleroomsvc@12345/HD_join_table", []byte(`{"table_id":"testtable01"}`))
